...
select * from menu_item where  Active="Yes" && year(Date_of_Launch)>2021 ;

/* Edit Menu Item (TYUC003) */
select * from menu_item  where  id=1 ;
update  menu_item set Name="ice cream",Price=150,Active="Yes",Date_of_Launch="2017-05-02",Category="Dessert",Free_Delivery="Yes" where id=1;
...
(1,1,1,"Sandwich","Yes",99,"Main Course"),
(1,2,2,"Burger","No",129,"Main Course"),
(1,3,3,"pizza","No",149,"Main Course");

/* View Cart (TYUC005) */
select c.cart_id,c.Name,c.Free_delivery,c.price,c.category from menu_item m join cart c on m.id=c.m_id where c.u_id=1;
...
delete from cart where u_id=1 and m_id=1;
select * from cart;